...
class OrderDataAccessTest {
    ...
    void existsOrderByItemCode() throws SQLException {
        assertDoesNotThrow(()->assertFalse(OrderDataAccess.existsOrderByItemCode("Item Code")));

        ItemDataAccess.saveItem(new Item("II12345678", "Item1", 5, new BigDecimal("1250")));
        CustomerDataAccess.saveCustomer(new Customer("ABC", "Pasindu", "Panadura"));
        SingleConnectionDataSource.getInstance().getConnection().createStatement()
                .executeUpdate("INSERT INTO \"order\" (id, customer_id) VALUES ('111111', 'ABC')");
        SingleConnectionDataSource.getInstance().getConnection().createStatement()
